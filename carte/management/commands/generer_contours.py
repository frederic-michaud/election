...
Les fichiers produits sont committés : la commande ne resert qu'au changement de
millésime. Deux sources, à télécharger :

1. swissBOUNDARIES3D de swisstopo, GeoPackage LV95 (du SQLite, lu sans dépendance) :
   ...
   Millésime suivant : API STAC, collection ``ch.swisstopo.swissboundaries3d``.
2. Le TopoJSON de l'OFS, pour les lacs — swissBOUNDARIES3D n'en a pas, une commune
   riveraine s'y étend jusqu'au milieu de l'eau :
   ``https://dam-api.bfs.admin.ch/hub/api/dam/assets/36438312/master``
   Millésime suivant : API CKAN d'opendata.swiss, jeu
   ``geodaten-zu-den-eidgenoessischen-abstimmungsvorlagen``.

La simplification garde la topologie : chaque frontière partagée est simplifiée
une seule fois pour les deux communes qui la bordent, sinon un liseré blanc
apparaîtrait entre voisines.
"""
...
class Simplificateur:
    ...
    def anneau(self, anneau):
        ...
        if not jonctions:
            # Anneau sans jonction (île, enclave) : toujours le même point de
            # départ, pour que les deux communes le simplifient pareil.
        ...
        # L'arrondi peut confondre deux sommets voisins ; un anneau sans surface est jeté.
        propre = [sortie[0]]
