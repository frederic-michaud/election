"""Plan factoriel 2^3 : ponderation de l'ACP x nombre de composantes x effet taille.

Les trois leviers identifies, croises. L'effet taille entre comme indicatrices
...
def main():
    ...
    for rang, cible in enumerate(cibles, 1):
        ...
        # log10 des electeurs de la votation precedente : connu le jour J,
        # centre-reduit pour ne pas degrader le conditionnement.
        ...
        for pondere in (False, True):
            for k in (6, 20):
                for avec_taille in (False, True):
                    bloc = [comp[pondere][:, :k]]
                    if avec_taille:
                        bloc.append(lt)
                    bloc.append(np.ones((nb, 1)))
                    designs[(pondere, k, avec_taille)] = np.column_stack(bloc)
    ...
    entetes = [f"{'p' if p else 'u'}{k}{'+t' if c else ''}" for p, k, c in variantes]
